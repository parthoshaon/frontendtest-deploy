...
const content = [
    ...
    },
    {
        ...
        icon: <svg width="176" height="80" viewBox="0 0 176 80" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const CTA = () => {
  return (
    <div className="grid text-[#0c0114] p-6 gap-8 lg:grid-cols-3 md:grid-cols-2 grid-cols-1">
        {content.map((item,i)=>(
            <div key={item.header} className="flex flex-col gap-8 p-6 items-center text-center border-2 boder-gray-200 rounded-lg mx-auto" data-aos={`zoom-${ i % 2 != 0 ? `in` : `out`}`}>
                {item.icon}
                    <h1>{item.header}</h1>
